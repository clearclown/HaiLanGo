...
- [x] RESTful API
  - 書籍作成
  - ファイルアップロード
  - チャンクアップロード（initiate, upload, status）⭐ NEW
  - 進捗取得
- [x] 包括的なテストカバレッジ
  - ユニットテスト
  - 統合テスト
...
- [x] チャンクアップロード対応 ✅
- [ ] **HEIC→PNG変換**
  - 注記: HEICファイルの変換には外部ライブラリが必要
  - 推奨ライブラリ: `github.com/strukturag/libheif` (CGO required)
  - または クライアントサイドでの変換を推奨
- [ ] S3互換ストレージ対応（Cloudflare R2, AWS S3）
