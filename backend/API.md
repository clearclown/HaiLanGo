...
システムがサポートしている言語コードの一覧を取得します。

**エンドポイント**: `GET /api/v1/languages`

**レスポンス** (200 OK):
```json
{
  "languages": ["ja", "en", "zh", "ru", "fa", "he", "es", "fr", "pt", "de", "it", "tr", "ar", "ko", "vi", "th", "hi"]
}
```
...
大容量ファイルをチャンクに分割してアップロードするための初期化を行います。

**エンドポイント**: `POST /api/v1/books/:book_id/chunk/initiate`

**リクエストボディ**:
```json
{
  "file_name": "large_book.pdf",
  "total_chunks": 20,
  "file_size": 104857600
}
```
...
**フィールド**:
- `file_name` (必須): ファイル名
- `total_chunks` (必須): 総チャンク数
- `file_size` (必須): ファイルサイズ（バイト）
...
**エンドポイント**: `POST /api/v1/books/:book_id/chunk/:upload_id/:chunk_number`

**パラメータ**:
- `book_id` (必須): 書籍ID（UUID）
- `upload_id` (必須): チャンクアップロードID（UUID）
- `chunk_number` (必須): チャンク番号（0から開始）
...
```bash
curl -X POST \
  http://localhost:8080/api/v1/books/550e8400.../chunk/770e8400.../0 \
  -F "chunk=@chunk_0.bin"
```
...
チャンクアップロードの進捗状況を取得します。

**エンドポイント**: `GET /api/v1/books/:book_id/chunk/:upload_id/status`
...
```json
{
  ...
  "status": "uploading",
  "created_at": "2025-11-13T08:00:00Z",
  "updated_at": "2025-11-13T08:05:00Z"
}
```

**ステータス値**:
- `pending`: 初期化済み、アップロード待ち
- `uploading`: アップロード中
- `completed`: 完了（全チャンク結合済み）
- `failed`: 失敗
