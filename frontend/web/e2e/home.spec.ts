import { expect, mockUser, test } from './fixtures';
...
test.describe('Home Page', () => {
  ...
  test('should have continue learning link', async ({ page }) => {
    const link = page.getByRole('link', { name: /続きから学習/ });
    await expect(link).toBeVisible();
  });

  test('should display quick access section', async ({ page }) => {
    await expect(page.getByText(/マイ本/).first()).toBeVisible();
    await expect(page.getByText(/復習/).first()).toBeVisible();
  });
