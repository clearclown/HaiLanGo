...
export function PatternPractice({
  ...
  const allAnswers = currentPractice
    ? [currentPractice.correct_answer, ...currentPractice.alternative_answers].sort(
        () => Math.random() - 0.5
      )
    : [];
  ...
  const handleAnswerClick = (answer: string) => {
    ...
    setTimeout(() => {
      if (currentIndex < practices.length - 1) {
        ...
        // Completed all questions - increment to trigger isCompleted
        setCurrentIndex(currentIndex + 1);
        if (onComplete) {
          onComplete(newScore);
        }
  ...
  return (
    <div className="max-w-2xl mx-auto p-8">
      ...
      {/* Question */}
      <div className="mb-8">
        <h2 className="text-2xl font-semibold mb-6">{currentPractice.question}</h2>
          ...
          {allAnswers.map((answer, index) => {
            ...
            return (
              ...
              >
                ...
                {isAnswered && isSelected && <span className="ml-2">{isCorrect ? '✓' : '✗'}</span>}
              </button>
            );
          })}
        </div>
        ...
        >
          {selectedAnswer === currentPractice.correct_answer ? (
            ...
            <p className="font-semibold">
              Not quite. The correct answer is: {currentPractice.correct_answer}
            </p>
          )}
        </div>
      )}
